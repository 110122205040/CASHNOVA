...
const SalesTrendChart = ({ loading }) => {
  const { deleteEntry, entries, formatCurrency, saving, updateEntry } = useAppData();
  ...
  const filteredEntries = useMemo(() => {
    if (filterMode === "date") {
      return entries.filter((entry) => getDateKey(entry.date) === selectedDate);
    }
  ...
  }, [entries, filterMode, selectedDate, selectedMonth]);
